...
def get_batch_frames(args, video_paths, tensorizer):
    ...
    for video_path in video_paths:
        ...
        preproc_frames = _transforms(args, frames)
        data_sample = tensorizer.tensorize_example_e2e('', preproc_frames)
        data_sample = list(t.to(args.device) for t in data_sample)
...
def run_inference_on_one(args, data_sample, model, tokenizer, tensorizer,
    ...
    inputs['token_type_ids'] = data_sample[2][None,:]
    inputs['img_feats'] = data_sample[3][None,:]
    inputs['masked_pos'] = data_sample[4][None,:]
...
def batch_inference(args, video_paths, model, tokenizer, tensorizer):
    ...
    # data_sample = tensorizer.tensorize_example_e2e('', preproc_frames)
    # data_sample = tuple(t.to(args.device) for t in data_sample)
    batch = get_batch_frames(args, video_paths, tensorizer)
...
def update_existing_config_for_inference(args):
    ...
    train_args.model_name_or_path = './models/captioning/bert-base-uncased/' #TODO
...
def get_custom_args(base_config):
    ...
    parser.add_argument('--video_batch', action='store_true')
    parser.add_argument('--test_video_batch_dname', type=str, default='None')
...
def set_args_for_batch(args, checkpoint = "./models/table1/youcook2/best-checkpoint/model.bin",
    ...
    args.eval_model_dir = eval_dir
    args.test_video_batch_dname = video_batch_folder
...
def get_dir_files(dir='./docs'):

  f = []
  for (dirpath, dirnames, filenames) in os.walk(dir):
    filenames = [os.path.join(dir, x) for x in filenames]
    f.extend(filenames)
    break
  return f
